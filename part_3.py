...
#     ---------------------------------------CAMBIOS-----------------------------------------
    ...
    wfpp = Wofost72_PP(params, wdp_conv, agro)    #Wofost72_PP (potencial production). It only takes into account the effect of irradiance, assuming that the other parameters that influence growth are always optimal. This way, we can compare the different results while only considering the irradiance, which is what is being investigated in this case.
    wfpp.run_till_terminate()
    summary_output = wfpp.get_summary_output()
